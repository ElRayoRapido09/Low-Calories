<script>
  ...
  let months = []; // array de {year, month, daysGrid}
  ...
  function generateMonth(y, m){
    ...
    const grid = new Array(totalCells).fill(null).map((_, i) => {
      ...
      return new Date(y, m, dayIndex);
    });
    return { year: y, month: m, grid };
  }
  ...
  function confirmRange(){
    // aquí mandar al backend o navegar con el rango
    if(rangeStart && rangeEnd){
      const payload = { start: rangeStart.toISOString(), end: rangeEnd.toISOString() };
      console.log('Rango seleccionado', payload);
      // ejemplo: goto('/algo', { state: payload })
    }
  }
</script>
...
<main class="app">
  ...
  <div class="bottom">
    ...
    <button class="btn primary" on:click={confirmRange} disabled={!rangeStart || !rangeEnd}>
      {#if rangeStart && rangeEnd}Seleccionar {rangeStart.toLocaleDateString()} – {rangeEnd.toLocaleDateString()}{:else}Selecciona un rango{/if}
    </button>
  </div>
</main>
...
<style>
 ...
 .cell:disabled{ opacity:0.3; pointer-events:auto; } /* keep disabled look but allow click blocked by logic */
